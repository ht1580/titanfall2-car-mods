...
VANILLA_PARTICLE_MANIFEST = ROOT / "research/vanilla/particles/particles_manifest.txt"
...
def euler_xyz_to_quat(v):
    ...
    return quat_mul(quat_mul((cx, sx, 0, 0), (cy, 0, sy, 0)), (cz, 0, 0, sz))


def quat_to_euler_xyz(q):
    w, x, y, z = q
    r00 = 1 - 2*(y*y + z*z); r01 = 2*(x*y - z*w); r02 = 2*(x*z + y*w)
    r12 = 2*(y*z - x*w); r22 = 1 - 2*(x*x + y*y)
    yy = math.asin(max(-1.0, min(1.0, r02)))
    if abs(math.cos(yy)) > 1e-7:
        xx = math.atan2(-r12, r22)
        zz = math.atan2(-r01, r00)
    else:
        xx = 0.0
        zz = math.atan2(2*(x*y + z*w), 1 - 2*(x*x + z*z))
    return (xx, yy, zz)
...
def add_idle_particle_events(qc_text: str, effect: str, attachment: str, stop_frame: int = 189) -> str:
    ...
    return pattern.sub(r'\1' + events, qc_text, count=1)
...
def build_mythic() -> dict:
    ...
    text = add_idle_particle_events(text, "codex_car_mythic_mflash_xo_elec_glow", "VFX_eye")
    ...
    metadata.update(Version="1.3.1-idleauto-pcf", LoadPriority=0, Description="Apex effect-bone Delta baked into the stock idle/ADS autoplay channels; full vanilla particle manifest retained and custom PCF appended.")
    ...
    archive = RELEASE / "CAR.Mythic.Allfather-1.3.1-idleauto-pcf.zip"
...
def build_double() -> dict:
    ...
    text = add_idle_particle_events(text, "codex_doubletake_hunter_mflash_xo_elec_glow", "hunter_fx")
    ...
    metadata_path = destination / "mod.json"
    metadata = json.loads(metadata_path.read_text(encoding="utf-8-sig"))
    metadata.update(Version="1.0.10-idleauto-pcf", LoadPriority=0, Description="Stock optics and five RUI meshes retained; ring Delta baked into the stock idle/ADS autoplay channels with a full vanilla particle manifest.")
    metadata_path.write_text(json.dumps(metadata, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    archive = RELEASE / "Codex.DoubleTake.HunterSafari-1.0.10-idleauto-pcf.zip"
    zip_folder(destination, archive)
